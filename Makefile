...
COMMON_FLAGS=-trimpath -ldflags="-w -s -buildid= -checklinkname=0" -buildvcs=false -gcflags=all=-trimpath="$(GOMODCACHE)/github.com/sagernet=>phantom-internal;$(shell pwd)=>phantom-core;github.com/phantomcloude/phantom-core=>phantom-core;$(TRIMPATH_REPLACEMENTS)"
GOBUILD_FLAGS=$(COMMON_FLAGS) -asmflags=all=-trimpath="$(GOMODCACHE)/github.com/sagernet=>phantom-internal;$(shell pwd)=>phantom-core;github.com/phantomcloude/phantom-core=>phantom-core;$(TRIMPATH_REPLACEMENTS)"
